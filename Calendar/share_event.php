<?php
    ...
    if ($same){
        $added = false;
        $stmt = $mysqli ->prepare("SELECT Event_name FROM events WHERE User_ID = ? AND Date_Due = ? AND Event_Name = ?");
        $stmt->bind_param('iss',$shared_id,$event_due,$event_name);
        $stmt->execute();
        $stmt->bind_result($existing_event);
        while ($stmt->fetch()){
        ...
        if (!$added){
            ...
            if(!$stmt){
                ...
                $stmt->bind_param('sisssis',$event_name,$shared_id,$current_time,$event_due,$event_description,$shared_id,$tags);
                if ($stmt->execute()){
            ...
            echo json_encode("The event has been added already!");
        }
    }
?>